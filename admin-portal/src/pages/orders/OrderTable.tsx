...
import { Search, Download, ArrowRight, ArrowLeft, User, Store, Loader2, Calendar, Eye } from 'lucide-react';
...
export default function OrderTable() {
  ...
  const filteredOrders = useMemo(() => {
    ...
    if (dateParam) {
      result = result.filter(o => {
        ...
        // created_at is likely ISO string, convert to YYYY-MM-DD
  ...
  const handleExportCSV = async () => {
    if (!dateParam && filteredOrders.length > 31 * 50) {
      // Very naive logic to represent the 31-day limit requirement
  ...
  const columns: Column<Order>[] = [
    {
      ...
      cell: (item) => {
        ...
        return (
          <div>
            <Link to={`/orders/${item.id}`} className="font-mono font-black text-brand-navy hover:text-brand-orange-600 transition-colors">
              #{item.id}
            </Link>
    ...
    },
    {
      ...
      cell: (item) => (
        <div className="flex items-center gap-2 font-bold text-brand-navy">
          <div className="p-1.5 bg-gray-50 rounded-md border border-border">
    ...
    },
    {
      ...
      cell: (item) => (
        <div>
          ...
          <div className="text-xs font-medium text-text-secondary mt-0.5 ml-5">
      ...
      cell: (item) => (
        <div className="flex justify-end pr-4">
          <Link to={`/orders/${item.id}`}>
            <Button variant="outline" className="gap-2 text-sm px-4 py-2 font-bold shadow-sm border border-border bg-white/80 backdrop-blur hover:bg-brand-orange-500 hover:text-white hover:border-brand-orange-500 rounded-xl transition-all group">
              <Eye className="w-4 h-4 text-text-secondary group-hover:text-white transition-colors" /> View
            </Button>
          </Link>
        </div>
      ),
