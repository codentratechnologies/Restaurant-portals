...
export default function OrderCalendar() {
 ...
 // 2. Listen to customers (scoped to this admin)
 useEffect(() => {
  if (!currentUser) return;
 ...
 type: 'Delivery',
 customer: { 
 name: customer.fullName || customer.name || rawOrder.customer?.name || rawOrder.customerName || 'Unknown Customer', 
...
function OrderCard({ order, onAccept, onReject, onAutoReject, disabled }: { order: OrderData, onAccept: () => void, onReject: () => void, onAutoReject: () => void, disabled: boolean }) {
 ...
 <p className="text-xs font-medium text-text-secondary mt-0.5">{order.customer.phone}</p>
 )}
 </div>
