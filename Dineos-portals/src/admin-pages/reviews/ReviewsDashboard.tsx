...
import { Star, Filter, MessageSquareText, ThumbsUp, ThumbsDown, Search, Eye, ChevronLeft, ChevronRight, ChevronDown } from 'lucide-react';
...
export default function ReviewsDashboard() {
  ...
  const rawReviews = useMemo(() => {
    return orders
      .filter((o) => o.customer_review)
      .map((o) => ({
        id: o.customer_review.id || `REV-${o.id}`,
        rating: o.customer_review.rating || 5,
        date: o.customer_review.date || new Date(o.created_at).toISOString().replace('T', ' ').slice(0, 16),
        customerName: o.customer?.name || 'Unknown Customer',
        isAnonymous: o.customer_review.isAnonymous || false,
        comment: o.customer_review.comment || '',
        orderId: o.id,
        orderedItems: (o.items || []).map((i) => ({ name: i.name, price: i.price })),
      }))
  ...
  const filteredReviews = useMemo(() => {
    ...
    if (selectedRating !== 'All') {
      res = res.filter(r => r.rating === selectedRating);
    }
    if (searchInput.trim()) {
    ...
    return res;
  }, [rawReviews, searchInput, selectedRating]);
