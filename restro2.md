...
| Field Name | Type | Required | Validation | Example | Notes |
|---|---|---|---|---|---|
| Order Ticket | Card | Yes | Read-only verification | `#ORD-99018` | Displays customer details, items, bill totals |
...
*   **Form**: Checkboxes or radio grids with active state highlight. Input text container has a character limit counter (0/250).
...
| Field Name | Type | Required | Validation | Example | Notes |
|---|---|---|---|---|---|
| Reason Code | Radio Group | Yes | Must match one of system enum cancellation keys | `out_of_stock` | Saved to logs database |
...
| Field Name | Type | Required | Validation | Example | Notes |
|---|---|---|---|---|---|
| Active Status | Selector | Yes | Read-only | `Preparing` | Moves ticket along columns |
...
#### 8. Database Table Suggestions
Re-uses columns from the main `branch_orders` mapping schema. Status updates write records to the `order_status_history` audit tracking ledger.
...
| Field Name | Type | Required | Validation | Example | Notes |
|---|---|---|---|---|---|
| Rating Filter | Selector | No | Must be 1 to 5, or 'All' | `1` | Filters display |
| Sort Order | Dropdown | Yes | Value must be 'Newest' or 'Oldest' | `Newest` | Sorts reviews timeline |
